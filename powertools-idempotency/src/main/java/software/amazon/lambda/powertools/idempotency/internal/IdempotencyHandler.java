...
/**
 * Internal class that will handle the Idempotency, and use the {@link
 * software.amazon.lambda.powertools.idempotency.persistence.PersistenceStore} to store the result
 * of previous calls.
 */
public class IdempotencyHandler {
    ...
    public IdempotencyHandler(
            ProceedingJoinPoint pjp, String functionName, JsonNode payload, Context lambdaContext) {
        this.pjp = pjp;
        this.data = payload;
    ...
    private Object processIdempotency() throws Throwable {
        try {
            // We call saveInProgress first as an optimization for the most common case where no
            // idempotent record
    ...
    /**
     * Tries to determine the remaining time available for the current lambda invocation. Currently,
     * it only works if the idempotent handler decorator is used or using {@link
    ...
    private OptionalInt getRemainingTimeInMillis() {
        if (lambdaContext != null) {
            ...
            LOG.warn(
                    "Couldn't determine the remaining time left. Did you call registerLambdaContext on Idempotency?");
        }
    ...
    private DataRecord getIdempotencyRecord() {
        try {
            ...
            // This code path will only be triggered if the record is removed between saveInProgress
            // and getRecord
            ...
            throw new IdempotencyInconsistentStateException(
                    "saveInProgress and getRecord return inconsistent results", e);
    ...
    private Object handleForStatus(DataRecord record) {
        ...
        // saveInProgress call and here
        if (EXPIRED.equals(record.getStatus())) {
            throw new IdempotencyInconsistentStateException(
                    "saveInProgress and getRecord return inconsistent results");
        }
        ...
        try {
            ...
            return JsonConfig.get()
                    .getObjectMapper()
                    .reader()
                    .readValue(record.getResponseData(), returnType);
        } catch (Exception e) {
            throw new IdempotencyPersistenceLayerException(
    ...
    private Object getFunctionResponse() throws Throwable {
        ...
        try {
            ...
            // We need these nested blocks to preserve function's exception in case the persistence
            // store operation
